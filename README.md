...
### 1.8.83
#### 修复、优化
- 修复网易云 Web 登录（改用移动端页面，修正登录成功判断逻辑）
- 网易云关注歌手/收藏专辑支持自动分页，获取全量数据
- 播放详情页无封面时导航动画降级为普通滑入，避免共享元素动画异常
- 修复网络歌词 UDP Socket 在不支持广播的设备上崩溃的问题
- 修复播放、下载速度异常（添加统一请求头）
- 下载管理器支持恢复中断的下载任务
- 关注歌手/收藏专辑页面支持横屏 2 列布局
- 日推分类列表支持滚动
- 网络歌词支持音量控制命令
- 登录页面触摸优化
---
